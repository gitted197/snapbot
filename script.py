...
def clickCameraOrContinue(device) -> bool:
    """
    Click the camera button, unless the snap was already taken.

    Returns True when this function already clicked the next/send button.
    Returns False when the caller still needs to click next/send.
    """
    try:
        ...
        logger.warning(
            "Camera button not found after click retries; checking if preview is already open."
        )

    try:
        ClickButton("clicking_next", device).ClickNow(NEXT_BUTTON_ID, None)
        logger.info("Preview screen detected; clicked next without retrying camera.")
        return True
    except RuntimeError:
        logger.warning(
            "Camera button and next button not found; rebooting Snapchat and trying camera once more."
        )
        rebootSnap(device)
        ClickButton("clicking_camera", device).ClickNow(CAMERA_CAPTURE_BUTTON_ID, None)
        return False
...
def mainScript(username_input: str, points_input_raw) -> tuple[int, float]:
    ...
    try:
        while pointscounter < points_input:
            ...
            # clicking steps
            next_already_clicked = clickCameraOrContinue(device)

            if not next_already_clicked:
                sleep(1)
                ClickButton("clicking_next", device).ClickNow(NEXT_BUTTON_ID, None)
